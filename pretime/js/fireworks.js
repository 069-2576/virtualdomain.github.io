...
render()
function render() {
    setTimeout(render, 1000 / 90)
    ctx.fillStyle = 'rgba(0, 0, 0, 0.1)';
    ctx.fillRect(0, 0, ctx.canvas
    .width, ctx.canvas
    .height)
    for (var firework of canvas) {
...
function Firework(x, y) {
    ...
    this.x = x || Math.random() * ctx.canvas
.width
    this.y = y || ctx.canvas
.height
    this.height = Math.random() * ctx.canvas
.height / 2
    this.dead = false
